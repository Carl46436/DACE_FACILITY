...
const QRScannerScreen = ({ navigation }) => {
  ...
  const lookupItem = async (qrCode) => {
    ...
    try {
      ...
      Alert.alert('Not Found', error.message || 'No item found for this QR code.');
      setScannedItem(null);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleBorrow = () => {
    if (scannedItem) {
      navigation?.navigate?.('Borrowing', { itemId: scannedItem.id, itemName: scannedItem.name });
    }
  };
              ...
              <View style={styles.cameraContainer}>
                ...
                <View style={styles.scanOverlay}>
                  ...
                  <Text style={styles.scanHint}>
                    {hasScanned ? 'Scan complete' : 'Align QR code inside the frame'}
                  </Text>
                  {hasScanned && (
                    <CustomButton
                      title="Scan Again"
        ...
        {scannedItem && (
          <View style={styles.itemCard}>
            ...
            <Text style={styles.itemMeta}>
              {scannedItem.category} | {scannedItem.status} | {scannedItem.location || 'No location'}
            </Text>
...
const styles = StyleSheet.create({
  ...
  cameraPlaceholder: {
    height: 220, backgroundColor: colors.surface, borderRadius: borderRadius.lg,
    justifyContent: 'center', alignItems: 'center', borderWidth: 2,
    borderColor: colors.border, borderStyle: 'dashed', marginBottom: spacing.lg,
  },
  placeholderText: {
    fontSize: fontSize.sm, color: colors.textMuted, textAlign: 'center', marginTop: spacing.md,
  },
  ...
  itemFound: { fontSize: fontSize.md, fontWeight: '600', color: colors.success },
  itemName: { fontSize: fontSize.xl, fontWeight: '700', color: colors.text },
  itemMeta: { fontSize: fontSize.sm, color: colors.textSecondary, marginTop: 4, textTransform: 'capitalize' },
  itemDesc: { fontSize: fontSize.sm, color: colors.textSecondary, marginTop: spacing.sm, lineHeight: 20 },
  itemActions: { marginTop: spacing.lg },
  unavailable: { fontSize: fontSize.sm, color: colors.warning, fontWeight: '500', textAlign: 'center' },
});
