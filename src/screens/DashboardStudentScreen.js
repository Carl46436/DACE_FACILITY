...
// Tailored dashboard for student users
// ============================================================
...
const DashboardStudentScreen = ({ navigation }) => {
  ...
  const loadDashboard = useCallback(async () => {
    try {
      const [reportsRes, borrowsRes, unreadRes] = await Promise.allSettled([
        api.reports.getAll({ page: 1, limit: 10, sort_by: 'created_at', sort_order: 'desc' }),
      ...
      const borrows = borrowsRes.status === 'fulfilled' ? (borrowsRes.value.data?.data || []) : [];
      const activeReports = reports.filter((report) => !['resolved', 'closed', 'rejected'].includes(report.status)).length;
      const pendingBorrows = borrows.filter((borrow) => borrow.status === 'pending').length;
      const activeBorrows = borrows.filter((borrow) => ['approved', 'active', 'overdue'].includes(borrow.status)).length;
      ...
      <View style={styles.section}>
        ...
        <View style={styles.tipCard}>
          ...
          <View style={styles.tipContent}>
            <Text style={styles.tipTitle}>Report tips</Text>
            <Text style={styles.tipText}>Include a clear title, exact location, and a photo if possible.</Text>
          </View>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  tipContent: { flex: 1 },
  tipTitle: { fontSize: fontSize.sm, fontWeight: '700', color: colors.text },
  tipText: { fontSize: fontSize.xs, color: colors.textSecondary, marginTop: 2, lineHeight: 18 },
  logoutButton: {
