...
import { View, Text, ScrollView, StyleSheet, Image, Alert, TextInput, TouchableOpacity } from 'react-native';
...
import { formatDateTime, formatRelativeTime, getInitials } from '../utils/formatters';
import { confirmAction } from '../utils/confirm';
import { api } from '../services/api';

const ReportDetailScreen = ({ navigation, route }) => {
  const reportId = route?.params?.id;
  const { currentReport, fetchReportById, updateReportStatus, addComment, isLoading } = useReports();
  const { user } = useAuth();
  ...
  const isStaff = ['maintenance_staff', 'admin', 'super_admin'].includes(user?.role);
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    if (isAdmin && newStatus === 'assigned') {
      if (!assigneeId) {
        Alert.alert('Select Assignee', 'Please choose a maintenance staff member.');
        return;
      }
      statusPayload.assigned_to = assigneeId;
    } else if (user?.role === 'maintenance_staff' && ['assigned', 'in_progress'].includes(newStatus)) {
  ...
  const statusSteps = ['submitted', 'under_review', 'assigned', 'in_progress', 'resolved', 'closed'];
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.card}>
            ...
            >
              {report.images.map((img) => (
                <Image key={img.id} source={{ uri: img.image_url }} style={styles.reportImage} />
              ))}
            </ScrollView>
          </View>
        )}
        ...
        <View style={styles.card}>
          ...
          <View style={styles.detailRow}>
            ...
            <Text style={styles.detailText}>Submitted {formatDateTime(report.created_at)}</Text>
          </View>
          {report.reporter && (
            <View style={styles.detailRow}>
              <Ionicons name="person" size={16} color={colors.textSecondary} />
              <Text style={styles.detailText}>By {report.reporter.full_name} ({report.reporter.role})</Text>
            </View>
          )}
          {report.assignee && (
            <View style={styles.detailRow}>
              <Ionicons name="construct" size={16} color={colors.textSecondary} />
              <Text style={styles.detailText}>Assigned to {report.assignee.full_name}</Text>
            </View>
          )}
          {report.resolved_at && (
            <View style={styles.detailRow}>
              <Ionicons name="checkmark-circle" size={16} color={colors.success} />
              <Text style={styles.detailText}>Resolved {formatDateTime(report.resolved_at)}</Text>
            </View>
          )}
        </View>
          ...
          <View style={styles.card}>
              ...
              <View style={styles.assigneeBlock}>
                ...
                <View style={styles.assigneeList}>
                  {staffOptions.map((staff) => (
                    <TouchableOpacity
                      key={staff.id}
                      style={[styles.assigneeChip, assigneeId === staff.id && styles.assigneeChipActive]}
            ...
            <View style={styles.actionsRow}>
              {report.status === 'submitted' && (
                <CustomButton title="Review" onPress={() => handleStatusUpdate('under_review')} size="sm" fullWidth={false} />
              )}
              {report.status === 'under_review' && (
                <CustomButton title="Assign" onPress={() => handleStatusUpdate('assigned')} size="sm" fullWidth={false} />
              )}
              {report.status === 'assigned' && (
                <CustomButton title="Start Work" onPress={() => handleStatusUpdate('in_progress')} size="sm" fullWidth={false} />
              )}
              {report.status === 'in_progress' && (
                <CustomButton title="Resolve" onPress={() => handleStatusUpdate('resolved')} variant="secondary" size="sm" fullWidth={false} />
              )}
              {report.status === 'resolved' && (
                <CustomButton title="Close" onPress={() => handleStatusUpdate('closed')} variant="outline" size="sm" fullWidth={false} />
              )}
              {['submitted', 'under_review', 'assigned'].includes(report.status) && (
                <CustomButton title="Reject" onPress={() => handleStatusUpdate('rejected')} variant="danger" size="sm" fullWidth={false} />
              )}
            </View>
          </View>
        )}
        ...
        {/* Comments */}
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Comments ({report.comments?.length || 0})</Text>
          {report.comments && report.comments.length > 0 ? (
            report.comments.map((c) => (
              <View key={c.id} style={styles.commentItem}>
                <View style={styles.commentAvatar}>
                  <Text style={styles.commentInitials}>{getInitials(c.user?.full_name)}</Text>
                </View>
                <View style={styles.commentContent}>
                  <View style={styles.commentHeader}>
                    <Text style={styles.commentAuthor}>{c.user?.full_name}</Text>
          ...
          <View style={styles.addComment}>
            ...
            />
            <TouchableOpacity
              style={[styles.sendButton, !comment.trim() && styles.sendDisabled]}
...
const styles = StyleSheet.create({
  ...
  stepLabel: { fontSize: fontSize.xs, color: colors.textMuted, textTransform: 'capitalize' },
  ...
  reportImage: { width: 260, height: 180, borderRadius: borderRadius.md, marginRight: spacing.sm },
  actionsRow: { flexDirection: 'row', gap: spacing.sm, flexWrap: 'wrap' },
  assigneeBlock: { marginBottom: spacing.md },
  assigneeLabel: { fontSize: fontSize.sm, color: colors.textMuted, marginBottom: spacing.sm },
  ...
  commentInitials: { fontSize: fontSize.xs, fontWeight: '700', color: colors.primary },
  ...
  commentText: { fontSize: fontSize.sm, color: colors.textSecondary, lineHeight: 18 },
  noComments: { fontSize: fontSize.sm, color: colors.textMuted, textAlign: 'center', padding: spacing.md },
  addComment: {
    flexDirection: 'row', alignItems: 'flex-end', marginTop: spacing.md,
    borderWidth: 1, borderColor: colors.border, borderRadius: borderRadius.md, paddingLeft: spacing.md,
  },
  commentInput: { flex: 1, fontSize: fontSize.sm, color: colors.text, paddingVertical: spacing.sm, maxHeight: 80 },
